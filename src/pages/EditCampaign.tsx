...
const EditCampaign = () => {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    const campaigns = JSON.parse(localStorage.getItem('campaigns') || '[]');
    const updatedCampaigns = campaigns.map((campaign: any) => campaign.id === id ? {
      ...campaign,
      ...updatedCampaign
    } : campaign);
  ...
  const simulateUploadProgress = (uploadId: number, index: number) => {
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        setTimeout(() => {
          setUploadQueue(prev => {
            const updatedQueue = prev.map(item => item.id === uploadId ? {
              ...item,
              status: 'completed',
              progress: 100
            } : item);
  ...
  const calculateTotalDuration = () => {
    if (videoList.length === 0) return "00:00:00";
  ...
  return <div className="min-h-screen bg-gray-50 font-roboto">
      {/* Header */}
        ...
        <form onSubmit={onSubmit} className="space-y-8">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="space-y-3">
              ...
              <Input id="campaignName" placeholder="Enter campaign name" value={formData.campaignName} onChange={e => setFormData(prev => ({
              ...prev,
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="space-y-3">
              ...
              <div className={cn("border-2 border-dashed rounded-lg p-4 text-center cursor-pointer transition-colors", dragActive ? "border-blue-500 bg-blue-50" : "border-gray-300 hover:border-blue-400 hover:bg-gray-50")} onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}>
                <FileVideo className="mx-auto h-8 w-8 text-gray-400 mb-2" />
                <Button type="button" variant="outline" size="sm" className="h-8 text-sm border-gray-300 hover:bg-gray-50" onClick={() => setMediaBrowserOpen(true)}>
                  Select
                </Button>
                {formData.file && <p className="text-sm text-blue-600 mt-2">
                    {formData.file.name}
                  </p>}
              </div>
            </div>
          ...
          {uploadQueue.length > 0 && <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mt-6">
              <h3 className="text-sm font-medium text-gray-900 mb-3">Upload Progress</h3>
              <div className="space-y-2">
                {uploadQueue.map(upload => <div key={upload.id} className="flex items-center gap-3 p-2 bg-gray-50 rounded">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs font-medium text-gray-900 truncate">{upload.title}</span>
                        <div className="flex items-center gap-1 flex-shrink-0">
                          {upload.status === 'completed' ? <CheckCircle className="h-3 w-3 text-green-500" /> : <span className="text-xs text-gray-500">{Math.round(upload.progress)}%</span>}
                          <Button variant="ghost" size="sm" onClick={() => removeFromQueue(upload.id)} className="h-4 w-4 p-0 hover:bg-gray-200">
                            <X className="h-2 w-2" />
                          </Button>
                        </div>
                      </div>
          ...
          {videoList.length > 0 && <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {videoList.map((video, index) => <TableRow key={video.id} draggable onDragStart={e => handleVideoRowDragStart(e, index)} onDragOver={handleVideoRowDragOver} onDrop={e => handleVideoRowDrop(e, index)} className={cn("cursor-move transition-colors", draggedVideoIndex === index && "opacity-50", "hover:bg-gray-50")}>
          ...
          {/* Ad Preview Section */}
          {formData.file && <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Ad Preview</h3>
              <div className="flex justify-center">
                <div className="border border-gray-300 rounded-lg p-4 bg-gray-50">
                  {formData.file.type.startsWith('image/') ? <img src={URL.createObjectURL(formData.file)} alt="Ad preview" className="max-w-sm max-h-64 object-contain rounded" /> : formData.file.type.startsWith('video/') ? <video src={URL.createObjectURL(formData.file)} controls className="max-w-sm max-h-64 rounded" /> : <div className="flex items-center justify-center w-64 h-32 bg-gray-200 rounded">
          ...
          <div className="flex justify-center gap-6 pt-4">
            <Button type="submit" className="px-8 py-3 text-sm font-medium rounded-lg bg-blue-600 hover:bg-blue-700 text-white transition-colors shadow-sm">Save Campaign</Button>
            <Button type="button" variant="outline" onClick={handleCancel} className="px-8 py-3 text-sm font-medium rounded-lg border-gray-300 text-gray-700 hover:bg-gray-50 transition-colors">
              Cancel
            </Button>
          </div>
        </form>
